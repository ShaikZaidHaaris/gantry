...
ARMS = {"base": "rt_base", "ego": "rt_ego", "shuffled": "rt_shuffled"}
...
def main() -> None:
    ...
    print("=" * 74)
    print(f"  three-arm ablation, closed-loop in RoboTwin: {TASK}")
    print("=" * 74)
    for arm in arms:
    ...
    assembled = assemble(reports, dataset=f"the ego upload, against {TASK}")
    print(as_markdown(assembled))

    (HERE / f"abl_report_{TASK}.md").write_text(as_markdown(assembled))
    (HERE / f"abl_report_{TASK}.json").write_text(
        json.dumps(
            {
                "task": TASK,
                "arms": {
    ...
    print(f"\nwritten to {HERE}/abl_report_{TASK}.md")
