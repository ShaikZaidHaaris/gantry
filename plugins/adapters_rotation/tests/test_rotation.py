...
def pose(encoding, *, gripper=False, name="action"):
    ...
    return action_channel(
        name, "eef_abs_pose", width=width, rotation_repr=encoding,
...
def test_identity_stays_identity():
    for encoding in sorted(KNOWN):
        ...
        assert np.allclose(to_matrix(from_matrix(matrices, encoding), encoding), matrices, atol=1e-9)
...
def test_the_antipode_is_resolved_rather_than_left_to_flip():
    ...
    assert np.allclose(
        from_matrix(to_matrix(flipped, "quat_wxyz"), "quat_wxyz"), q, atol=1e-9
    )
...
def test_position_and_gripper_pass_through_untouched():
    ...
    values = np.concatenate(
        [np.arange(4 * 3).reshape(4, 3) * 1.0, random_rotations(4), np.array([[0.2], [0.4], [0.6], [0.8]])],
        axis=1,
    )
...
def test_the_data_really_comes_through_re_encoded():
    ...
    wiring, verdict = bind(requires_channels("c", "policy", target), episode.schema,
                           AdapterRegistry([ROTATION]))
    assert verdict.ok
    out = adapt_episode(episode, wiring).array("action")
    assert out.shape == (5, 6)
    assert np.allclose(to_matrix(out[:, 3:], "axis_angle"), to_matrix(values[:, 3:], "quat_wxyz"),
                       atol=1e-6)
...
def test_a_bare_rotation_is_the_one_unambiguous_layout():
    bare = ChannelSpec("q", "vector", (4,), "float32",
                       discriminators=("rotation_repr",),
                       metadata={"rotation_repr": "quat_wxyz"})
    other = ChannelSpec("q", "vector", (4,), "float32",
                        discriminators=("rotation_repr",),
...
def test_an_unknowable_offset_is_refused_with_the_fix():
    ...
    padded = ChannelSpec("x", "vector", (9,), "float32",
                         discriminators=("rotation_repr",),
                         metadata={"rotation_repr": "quat_wxyz"})
    target = ChannelSpec("x", "vector", (8,), "float32",
                         discriminators=("rotation_repr",),
